...
export default function LoanDetailModal({ isOpen, onClose, loan, urlApi, refresh }) {
  if (!loan) return null;
  ...
  const handleToggle = async (installment) => {
    try {
      const response = await axios.put(
        `${urlApi}gerencia/installments/${installment.id}`,
  ...
  const handleDelete = () => {
    if (!window.confirm(`¿Eliminar el préstamo de ${formatCurrency(loan.amount)} a ${loan.collaborator_name}?`)) return;

    toast.promise(
      axios
        .delete(`${urlApi}gerencia/loans/${loan.id}`, {
          headers: authHeaders(),
        })
        .then((response) => {
  ...
  return (
    <Modal
      ...
      contentLabel="Detalle del préstamo"
      style={{
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold">{loan.collaborator_name}</h2>
      ...
      <div className="grid grid-cols-3 gap-3 text-center">
        <div className="border rounded-md p-2.5">
          <p className="text-base font-bold">{formatCurrency(loan.amount)}</p>
          <p className="text-[11px] text-muted-foreground">Total prestado</p>
        </div>
        <div className="border rounded-md p-2.5">
