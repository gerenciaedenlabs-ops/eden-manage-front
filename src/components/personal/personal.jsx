...
import AddTransactionModal from "@components/personal/modal/add-transaction-modal.jsx";
import AddBudgetItemModal from "@components/personal/modal/add-budget-item-modal.jsx";
import AddDebtModal from "@components/personal/modal/add-debt-modal.jsx";
import DebtDetailModal from "@components/personal/modal/debt-detail-modal.jsx";
import AddSavingsGoalModal from "@components/personal/modal/add-savings-goal-modal.jsx";
import AddSavingsEntryModal from "@components/personal/modal/add-savings-entry-modal.jsx";
import {
  getCategoryColor,
  formatCurrency,
  formatDate,
  authHeaders,
} from "@components/personal/personal-constants.js";

export default function Personal({ urlApi }) {
  ...
  const getSummary = useCallback(() => {
    axios
      .get(`${urlApi}personal/summary`, { headers: authHeaders() })
  ...
  const getBudgetItems = useCallback(() => {
    axios
      .get(`${urlApi}personal/budget-items`, { headers: authHeaders() })
  ...
  const getTransactions = useCallback(() => {
    axios
      .get(`${urlApi}personal/transactions`, { headers: authHeaders() })
  ...
  const getDebts = useCallback(() => {
    axios
      .get(`${urlApi}personal/debts`, { headers: authHeaders() })
      .then((response) => setDebts(response.data.data))
      .catch((error) => console.error(error));
  }, [urlApi]);

  const getSavingsGoals = useCallback(() => {
    axios
      .get(`${urlApi}personal/savings-goals`, { headers: authHeaders() })
  ...
  const refresh = useCallback(() => {
    ...
    getDebts();
    getSavingsGoals();
  }, [getSummary, getBudgetItems, getTransactions, getDebts, getSavingsGoals]);
  ...
  const handleToggleBudgetItem = async (item) => {
    try {
      const response = await axios.put(
        `${urlApi}personal/budget-items/${item.id}`,
        { active: !item.active },
        { headers: authHeaders() }
      );
  ...
  const handleDeleteBudgetItem = (item) => {
    ...
    toast.promise(
      axios
        .delete(`${urlApi}personal/budget-items/${item.id}`, {
          headers: authHeaders(),
        })
        .then((response) => {
  ...
  const handleDeleteTransaction = (transaction) => {
    ...
    toast.promise(
      axios
        .delete(`${urlApi}personal/transactions/${transaction.id}`, {
          headers: authHeaders(),
        })
        .then((response) => {
          if (response.data.status === "ok") {
  ...
  const handleConfirmOccurrence = (occurrence) => {
    ...
    toast.promise(
      axios
        .put(
          `${urlApi}personal/budget-occurrences/${occurrence.occurrence_id}/confirm`,
  ...
  const handleDismissOccurrence = (occurrence) => {
    toast.promise(
      axios
        .put(
          `${urlApi}personal/budget-occurrences/${occurrence.occurrence_id}/dismiss`,
          {},
          { headers: authHeaders() }
        )
        .then((response) => {
  ...
  const handleDeleteSavingsGoal = (goal) => {
    ...
    toast.promise(
      axios
        .delete(`${urlApi}personal/savings-goals/${goal.id}`, {
          headers: authHeaders(),
        })
        .then((response) => {
          if (response.data.status === "ok") {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Tu presupuesto personal — solo vos podés verlo y editarlo.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-bold">Deudas</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="grid grid-cols-3 gap-3 text-center">
              <div>
                <p className="text-lg font-extrabold">{formatCurrency(summary?.debts?.total_debt)}</p>
                <p className="text-[11px] text-muted-foreground">Total deuda</p>
              </div>
              <div>
                <p className="text-lg font-extrabold text-green-700">
                  {formatCurrency(summary?.debts?.total_paid)}
                </p>
                <p className="text-[11px] text-muted-foreground">Pagado</p>
              </div>
              <div>
                <p className="text-lg font-extrabold text-amber-700">
                  {formatCurrency(summary?.debts?.pending)}
                </p>
                <p className="text-[11px] text-muted-foreground">Pendiente</p>
              </div>
            </div>

            {summary?.debts?.upcoming_installments?.length > 0 && (
              <div className="space-y-1 pt-1">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                  Próximas cuotas
                </p>
                {summary.debts.upcoming_installments.map((i) => (
                  <div key={i.id} className="flex items-center justify-between text-xs">
                    <span className="text-muted-foreground">
                      {i.debt_title} &middot; cuota {i.installment_number} &middot; {formatDate(i.due_date)}
                    </span>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Ingresos o gastos que no se repiten.
            </p>
          </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          ...
          <Button size="sm" onClick={() => setOpenAddDebt(true)}>
            <Plus className="w-4 h-4 mr-2" />
            Registrar deuda
          </Button>
        </CardHeader>
          ...
          <Table>
            ...
            <TableBody>
              {debts.map((debt) => {
                ...
                const pending = debt.installments
                  .filter((i) => !i.paid)
                ...
                return (
                  ...
                  >
                    <TableCell className="text-sm font-medium">{debt.title}</TableCell>
                    <TableCell className="text-sm text-muted-foreground whitespace-nowrap">
                      {formatDate(debt.start_date)}
                    </TableCell>
                    <TableCell className="text-sm">
                      {paidCount}/{debt.installments.length}
                    </TableCell>
                    ...
                    >
                      {pending > 0 ? formatCurrency(pending) : "Pagada"}
                    </TableCell>
                  </TableRow>
                );
              })}
